    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between selection:bg-blue-500 selection:text-white">
      ...
      <header className="border-b border-slate-800 bg-slate-900/60 backdrop-blur-md px-6 py-4 flex items-center justify-between sticky top-0 z-30">
        ...
        >
          ...
          <span>Kembali ke Beranda Technotama</span>
        </Link>

        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-teal-400 animate-ping" />
          <span className="text-[11px] font-mono text-teal-400 font-semibold tracking-wider">
            ISO-X ENTERPRISE ACTIVE
          </span>
        </div>
      </header>
      ...
      <main className="flex-1 flex items-center justify-center p-4 sm:p-6 md:p-8">
        <div className="w-full max-w-2xl bg-slate-900/90 border border-slate-800 rounded-2xl shadow-2xl shadow-blue-950/40 overflow-hidden">
          {/* HEADER */}
          <div className="bg-gradient-to-r from-blue-950 via-slate-900 to-teal-950 px-6 sm:px-8 py-6 border-b border-slate-800 flex items-center gap-4">
            <div className="p-3 rounded-2xl bg-gradient-to-br from-blue-600 to-teal-500 text-white shadow-xl shadow-blue-500/30">
              <ShieldCheck className="w-8 h-8" />
            </div>
            <div>
              <div className="flex items-center gap-2.5 flex-wrap">
                <h1 className="text-xl font-extrabold text-white tracking-wide">ISO-X</h1>
              ...
              <p className="text-xs sm:text-sm text-slate-400 mt-1">
                Integrated Management System, POJK Compliance & Matriks Wewenang JL7/JL8/JL9
              </p>
            </div>
          </div>
                ...
                <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                  <div>
                    <h2 className="text-sm font-bold text-white flex items-center gap-2">
                      ...
                      Autentikasi Portal Konsultan & Pengawas Kepatuhan
                    </h2>
                    <p className="text-xs text-slate-400 mt-0.5">
                      Silakan pilih persona konsultan atau masukkan kredensial korporat Anda.
                    </p>
                  </div>
                      ...
                      return (
                        <button
                          ...
                          className={`text-left p-3 rounded-lg border transition-all text-xs flex flex-col justify-between ${
                            isSelected
                              ? 'bg-blue-950/40 border-blue-500/60 text-white shadow-md shadow-blue-500/20'
                ...
                <form onSubmit={handleCredentialSubmit} className="space-y-4">
                  ...
                  <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3">
                    ...
                    <button
                      type="submit"
                      disabled={isLoading}
                      className="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-teal-600 hover:from-blue-500 hover:to-teal-500 text-white text-xs font-bold transition-all shadow-md shadow-blue-600/30 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                    >
                      {isLoading ? (
                        <>
              ...
              <div className="space-y-6">
                <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                      Tingkat otorisasi: <span className="font-bold text-white">{selectedUser.jlLevel}</span> untuk personel <span className="text-white font-medium">{selectedUser.name}</span>.
                    </p>
                  </div>
                  <span className="text-[11px] font-mono px-2.5 py-1 rounded bg-slate-800 text-slate-400 border border-slate-700">
                    Tahap 2/2
                ...
                <form onSubmit={handleMfaSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="987654"
                      className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-3 text-center text-xl tracking-[0.5em] font-mono text-teal-300 focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500"
                      required
                    />
                    <p className="text-[11px] text-slate-500 mt-1.5 text-center">
                      Mendukung Google Authenticator, Microsoft Authenticator, atau FIDO2 Security Token.
                    </p>
                  </div>
            ...
            {step === 'authenticated' && (
              <div className="space-y-6">
                <div className="p-4 rounded-xl bg-teal-950/30 border border-teal-500/40 text-teal-200 text-xs flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-bold text-white text-sm">Sesi Konsultan ISO-X Aktif</div>
                    <p className="text-slate-300 mt-0.5">
                      Autentikasi tingkat <span className="text-teal-300 font-semibold">{selectedUser.jlLevel}</span> untuk <span className="text-white font-medium">{selectedUser.name}</span> berhasil.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2.5">
                  <div className="text-xs font-semibold text-slate-300 flex items-center justify-between">
                    <span>Modul Sistem ISO-X Siap Digunakan:</span>
                ...
                <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3">
                  ...
                  <Link
                    href="/"
                    className="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold transition-colors text-center"
                  >
          ...
          {/* FOOTER */}
          <div className="bg-slate-950 px-6 sm:px-8 py-3.5 border-t border-slate-800 flex flex-wrap items-center justify-between text-xs text-slate-500">
            <div className="flex items-center gap-3">
              <span>ISO 27001:2022</span>
              <span>&bull;</span>
              <span>POJK No. 38/POJK.03/2016</span>
              <span>&bull;</span>
              <span>TLS 1.3 Hardened</span>
            </div>
            <div className="text-slate-400 font-mono text-[11px]">
              PT Technotama Artha Raya
            </div>
          </div>
        </div>
      </main>
      ...
      <footer className="py-4 text-center text-xs text-slate-600 border-t border-slate-900">
        &copy; {new Date().getFullYear()} PT Technotama Artha Raya &bull; ISO-X Enterprise Platform
      </footer>
    </div>
  );
}
